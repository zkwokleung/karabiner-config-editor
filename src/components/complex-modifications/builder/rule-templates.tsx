...
const RULE_TEMPLATES: Array<{
...
}> = [
  {
    ...
    description:
      'Remap Caps Lock to Escape when pressed alone, Control when held',
    category: 'Basic',
    rule: {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-w-3xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            Choose from common Karabiner configurations to get started quickly
          </DialogDescription>
        </DialogHeader>
            ...
            {categories.map((category) => (
              <div key={category} className='space-y-3'>
                ...
                <div className='grid gap-3'>
                  {RULE_TEMPLATES.filter((t) => t.category === category).map(
                    (template, index) => (
                      <Card
                        key={index}
