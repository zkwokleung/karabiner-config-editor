import type { KarabinerConfig, Profile, Rule, Manipulator } from "@/types/karabiner"
...
function validateProfile(profile: Profile, index: number): ValidationError[] {
  ...
  if (profile.simple_modifications) {
    profile.simple_modifications.forEach((mod, modIndex) => {
      if (!mod.from?.key_code && !mod.from?.consumer_key_code && !mod.from?.pointing_button) {
        errors.push({
  ...
  if (profile.complex_modifications?.rules) {
    profile.complex_modifications.rules.forEach((rule, ruleIndex) => {
      errors.push(...validateRule(rule, `${path}.complex_modifications.rules[${ruleIndex}]`))
    })
  }
...
function validateRule(rule: Rule, path: string): ValidationError[] {
  ...
  if (!rule.manipulators || rule.manipulators.length === 0) {
    ...
    rule.manipulators.forEach((manipulator, manipulatorIndex) => {
      errors.push(...validateManipulator(manipulator, `${path}.manipulators[${manipulatorIndex}]`))
    })
  }
 ...
 */
function validateManipulator(manipulator: Manipulator, path: string): ValidationError[] {
  const errors: ValidationError[] = []
  ...
  if (!manipulator.from) {
    ...
    if (!manipulator.from.key_code && !manipulator.from.consumer_key_code && !manipulator.from.pointing_button) {
      errors.push({
        path: `${path}.from`,
        message: "From event must specify key_code, consumer_key_code, or pointing_button",
        severity: "error",
      })
    }
  }
  ...
  const hasToEvent =
    manipulator.to || manipulator.to_if_alone || manipulator.to_if_held_down || manipulator.to_after_key_up

  if (!hasToEvent) {
    errors.push({
      path: `${path}`,
      message: "Manipulator should have at least one 'to' event (to, to_if_alone, to_if_held_down, or to_after_key_up)",
      severity: "warning",
    })
  }
...
export function findDuplicateSimpleModifications(profile: Profile): string[] {
  ...
  profile.simple_modifications?.forEach((mod) => {
    const key = mod.from.key_code || mod.from.consumer_key_code || mod.from.pointing_button
    if (key) {
      if (seen.has(key)) {
        duplicates.push(key)
      }
...
export function findConflictingManipulators(rules: Rule[]): string[] {
  ...
  const seen = new Map<string, { ruleIndex: number; manipulatorIndex: number; ruleDescription: string }>()

  rules.forEach((rule, ruleIndex) => {
    rule.manipulators.forEach((manipulator, manipulatorIndex) => {
      // Create a unique key for this manipulator based on from event
      const fromKey =
        manipulator.from.key_code || manipulator.from.consumer_key_code || manipulator.from.pointing_button
      const mandatory = manipulator.from.modifiers?.mandatory?.sort().join("+") || ""
      const optional = manipulator.from.modifiers?.optional?.sort().join("+") || ""
      ...
      if (seen.has(uniqueKey)) {
        ...
        if (previous.ruleIndex !== ruleIndex) {
          const modifierText = mandatory ? ` + ${mandatory.replace(/\+/g, " + ")}` : ""
          conflicts.push(
        ...
        seen.set(uniqueKey, { ruleIndex, manipulatorIndex, ruleDescription: rule.description || "Unnamed rule" })
      }
    })
  })
