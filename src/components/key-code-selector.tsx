...
import { KARABINER_KEYCODES, getKeyCodeValue, findKeyCodeItem, type KeyCodeItem } from "@/lib/karabiner-keycodes"
...
export function KeyCodeSelector({
  ...
  const filteredCategories = KARABINER_KEYCODES.map((category) => ({
    ...
    items: category.items.filter((item) => {
      ...
      return item.label.toLowerCase().includes(search) || getKeyCodeValue(item).toLowerCase().includes(search)
    }),
  })).filter((category) => category.items.length > 0)

  const isCategorySelected = (categoryName: string) => {
    const category = filteredCategories.find((c) => c.category === categoryName)
        ...
        <div className="flex" style={{ height: `${menuHeight}px` }}>
          ...
          <ScrollArea className="flex-1">
            <div className="p-1">
              {hoveredCategory ? (
                (() => {
                  const category = filteredCategories.find((c) => c.category === hoveredCategory)
