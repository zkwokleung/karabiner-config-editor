...
export function FnFunctionKeysEditor({ fnKeys, onChange }: FnFunctionKeysEditorProps) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Remap function keys (F1-F12) behavior</p>
        </div>
          ...
          {fnKeys.length === 0 && (
            <Card className="p-8">
              <p className="text-sm text-muted-foreground text-center">
                No function key mappings yet. Add one to customize F1-F12 behavior.
              </p>
            </Card>
          )}
          ...
          {fnKeys.map((fnKey, index) => (
            <Card key={index} className="p-4">
              <div className="flex items-center gap-4">
                ...
                <Button size="icon" variant="ghost" onClick={() => deleteFnKey(index)}>
